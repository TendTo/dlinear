...
\email{e.casablanca2@newcastle.ac.uk, martin.sidaway@icloud.com} \and % TODO: add Martin's email and ensure orcidID
...
\begin{abstract}
    ...
    \keywords{SMT \and Delta-complete \and Floating point arithmetic \and Linear real arithmetic}
\end{abstract}
...
\begin{equation}
    ...
    \end{split}
\end{equation}
%\eqcaption{\gls{lp} problem in standard form.} % no captions for equations
...
\end{gather*}
%    \\
%    \noindent\text{will be split into}
will be split into
%    \\
\begin{gather*}
    \begin{pmatrix}
...
\begin{algorithm}
    ...
    \begin{algorithmic}[1]
        ...
        \If{$E \ne \emptyset$} \label{alg:theory-solver:infeasible}
        \State \Return $E$
        \EndIf
        ...
        \EndFor
        \For{$c \gets 0$ to $2^{|D|}$} \label{alg:theory-solver:loop}
        ...
        \If{$E = \emptyset$} \Comment{At least one subproblem is feasible} \label{alg:theory-solver:feasible}
        ...
        \If{$|V_T| = 2^{|T|}$} \Comment{Having explored it fully, a subset of the $\ne$ constraints is infeasible} \label{alg:theory-solver:relaxed-infeasible}
        \State \Return $E_T$
        \EndIf
        \EndFor
        \State \Return $E$
...
\textcolor{red}{footnote should be added after comma or dot.}

\textcolor{red}{You are mixing British English and American English. See the word summarized above. I have changed the language for spell checking. It is American English. Please check the whole document.}
...
\begin{figure}[H]
    ...
    \caption{LP benchmark results. The vertical axes show the cumulative number of problems solved by each solver considering only the 847 problems solved by all solvers (\textbf{left}) and considering all 1237 problems (\textbf{right}). \textcolor{red}{change the axis range to reduce the white space inside the left figure. The figures have low quality saved as png. Could you create them with higher quality or as pdf?}}
...
%     \end{subfigure}
%     \caption{SMT benchmark results. The vertical axes show the cumulative number of problems solved by each solver considering only the 823 problems solved by all solvers (\textbf{left}) and considering all 1754 problems (\textbf{right}). \textcolor{red}{Could you create them with higher quality or as pdf?}}
...
%\section{Integrated Neural network verification}
%\label{sec:nn-verification}
\section{Future Extensions}

%\textcolor{red}{What do we do with this part? We don't report any NN results...}

\textcolor{red}{Add a para on your ideas for improving efficiency of the tool.}
