...
\begin{algorithm}
    ...
    \begin{algorithmic}
        ...
        \State $N \gets \{a_i \in A_i : s_i \in \{\ne\}\}, N \in R$ \Comment{Set of not equal to constraints}
        \State $S \gets \{a_i \in A_i : s_i \in \{<\}\}$ \Comment{Set of strict inequalities constraints}
        \State $c \gets 0$
        \While{$c \le 2^{|N|}$}
        ...
        \State SIMPLEX($A, b, s, B$) \Comment{Solve the LP problem}
        \State $c \gets c + 1$
        \EndWhile
    \end{algorithmic}
\end{algorithm}
