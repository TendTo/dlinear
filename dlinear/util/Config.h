...
class Config {
 public:
  ...
  enum class LPSolver {
    SOPLEX = 0,   ///< Soplex solver
    QSOPTEX = 1,  ///< Qsoptex solver
  };
  /** Default phase for the SAT solver. */
  enum class SatDefaultPhase {
    False = 0,              ///< Use the false phase as the default
    True = 1,               ///< Use the true phase as the default
    JeroslowWang = 2,       ///< Use the JeroslowWang phase as the default
    RandomInitialPhase = 3  ///< Use a random initial phase
  };
  /** Format of the input file. */
  enum class Format {
    AUTO = 0,  ///< Determine the type of the input based on the file extension
    SMT2 = 1,  ///< SMT2 format
    MPS = 2,   ///< MPS format
  };
  /** LP mode used by the LP solver. */
  enum class LPMode {
    AUTO = 0,                       ///< Let the solver decide the mode
    PURE_PRECISION_BOOSTING = 1,    ///< Pure precision boosting.
    PURE_ITERATIVE_REFINEMENT = 2,  ///< Pure iterative refinement. Only available for @ref LPSolver::SOPLEX
    HYBRID = 3,                     ///< Hybrid mode. Only available for @ref LPSolver::SOPLEX
  };
 ...
 public:
   ...
   * It is true when the input format is Format::SMT2\, false if Format::MPS }
   */
