...
namespace dlinear {
    ArgParser::ArgParser() : parser_{PROG_NAME, PROG_VERSION} {
        addOptions();
    }
    ...
    void ArgParser::addOptions() {
        parser_.add_argument("--verbosity")
                .help("set verbosity level. 0 for critical, 1 for error, 2 for warn, 3 for info, 4 for debug, 5 for trace. Any other value will disable logging.")
                .default_value(0)
