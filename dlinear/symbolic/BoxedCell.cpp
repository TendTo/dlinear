...
BoxedCell::BoxedCell(const std::shared_ptr<const ExpressionCell>& expr)
    : kind_{expr->kind()}, value_{std::make_shared<const ExpressionCell>(expr)} {}

BoxedCell& BoxedCell::operator=(const std::shared_ptr<ExpressionCell>& new_expr) {
...
BoxedCell& BoxedCell::operator=(const std::shared_ptr<mpq_class>& new_value) {
  DLINEAR_ASSERT(is_constant(), "Box must contain a constant value");
  value_ = new_value;
  return *this;
}
