...
Expression NegateAddition(const Expression& e) {
  DLINEAR_ASSERT(is_addition(e), "Must be an addition");
...
Expression NegateMultiplication(const Expression& e) {
  DLINEAR_ASSERT(is_multiplication(e), "Must be a multiplication");
...
ExpressionCell& Expression::mutable_cell() {
  DLINEAR_ASSERT(cell_.use_count() == 1, "There must be only one box pointing to the cell");
...
Expression Expression::EvaluatePartial(const Environment& env) const { return cell().EvaluatePartial(env); }
...
void Expression::SubImpl(const Expression& rhs) {
  ...
  DLINEAR_ASSERT(!(is_constant(lhs) && is_constant(rhs)));
  // x - y => x + (-y)
  lhs += -rhs;
}
...
Expression clamp(const Expression& v, const Expression& lo, const Expression& hi) {
  DLINEAR_ASSERT(!is_constant(lo) || !is_constant(hi) || (lo <= hi));
...
void DoEnumerateMultiIndex(const int order, const int num_vars, const int start_dim, const MultiIndex& base,
  ...
  DLINEAR_ASSERT(order > 0);
  DLINEAR_ASSERT(start_dim >= 0);
  DLINEAR_ASSERT(base.size() == static_cast<size_t>(num_vars));
  if (order == 0) {
    return;
  }
  if (order == 1) {
...
Expression Exp(const vector<Expression>& terms, const MultiIndex& alpha) {
  DLINEAR_ASSERT(terms.size() == alpha.size());
...
void DoTaylorExpand(const Expression& f, const Environment& a, const vector<Expression>& terms, const int order,
  ...
  DLINEAR_ASSERT(order > 0);
  DLINEAR_ASSERT(terms.size() == static_cast<size_t>(num_vars));
